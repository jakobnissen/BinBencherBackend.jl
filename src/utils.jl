function tab_pairs(lines)
    lines |> imap(strip) |> ifilter(!isempty) |> imap() do line
        cu = codeunits(line)
        t1 = findfirst(isequal(UInt8('\t')), cu)
        t1 === nothing && error("No tabs in line $line")
        t2 = findnext(isequal(UInt8('\t')), cu, t1+1)
        t2 === nothing || error("More than two tab-sep fields in $line")
...
function binsplit_tab_pairs(t_pairs, sep::Union{Char, AbstractString})
    t_pairs |> imap() do (binname, seqname)
        ...
        p === nothing && error("Seperator $sep not found in seq name $seqname")
